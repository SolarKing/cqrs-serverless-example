...
let client; // persisted db connection
...
const mongoAdapterBuilder = {
  build: async ({ entityName }, { url, dbName } = {}) => {
    ...
    return {
      ...
      batchGet: async (ids = []) => {
        const foundProjections = await projectionCollection
          .find({ $or: ids.map((id) => ({ id })) })
          .toArray();
      ...
      setMetadata: async ({ version, state }) => {
        await metadataCollection.updateOne({ id: 'metadata' }, {
          $set: {
            state,
            version,
          },
